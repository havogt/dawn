...
private:
  ...
  void initialize_S() {
    // all fields supported by dawn are 2 (or 3 with sparse) dimensional: (unstructured, lev,
    // sparse) S has dimensions (unstructured, [MMX/MMY])
...
public:
  ...
  atlas::Field const& S_MXX() const { return m_S_MXX; }
  atlas::Field const& S_MYY() const { return m_S_MYY; }
  atlas::Field const& vol() const { return m_vol; }
  atlas::Field const& sign() const { return m_sign; }
  ...
  void fillInputData(atlas::Field const& field) const {
    ...
    atlas::Field m_rlonlatcr{fs_nodes_.createField<double>(
        atlas::option::name("rlonlatcr") | atlas::option::variables(edges_per_node))};
    auto rlonlatcr = atlas::array::make_view<double, 3>(m_rlonlatcr);

    atlas::Field m_rcoords{fs_nodes_.createField<double>(atlas::option::name("rcoords") |
...
int main() {
  ...
  atlasInterface::Field<double> vol = atlas::array::make_view<double, 2>(driver.vol());
  atlasInterface::Field<double> S_MXX = atlas::array::make_view<double, 2>(driver.S_MXX());
  atlasInterface::Field<double> S_MYY = atlas::array::make_view<double, 2>(driver.S_MYY());
